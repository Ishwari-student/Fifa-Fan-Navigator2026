...
*For safety, always look up at physical signage, never enter restricted zones, and avoid unlicensed private taxis.*`;
  ...
  try {
    ...
    // Dynamically override instructions based on user language choice
    let activeInstructions = SYSTEM_INSTRUCTION;
    if (language) {
      activeInstructions += `\n\nCRITICAL CONTEXT: The user's active language preference is set to: "${language}". 
You MUST speak/reply entirely in this language preference! 
- If 'ES' is selected, speak in friendly Argentinian Spanish.
- If 'KO' is selected, speak in polite Korean.
- If 'JA' is selected, speak in polite Japanese.
- If 'AR' is selected, speak in clear Egyptian Arabic.
- If 'EN' is selected, speak in English.
Ensure all helpful guidelines, concessions, safety warnings, and route instructions are translated correctly to match this tongue. Keep key terms easily recognizable.`;
    }
